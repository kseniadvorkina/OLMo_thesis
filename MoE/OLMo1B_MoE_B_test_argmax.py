...
expert_dirs = {
    "[1710-1780]": "./fine_tuned_model_full_B_1710_1780",
    "[1780-1850]": "./fine_tuned_model_full_B_1780_1850",
    "[1850-1920]": "./fine_tuned_model_full_B_1850_1920"
}
...
moe_model.gate.load_state_dict(torch.load("./moe_gate_3epochs_B_epoch3.pt", map_location=device))
moe_model.eval()
...
def prepare_chunks(text, max_context_size=2048):
    max_chunk = max_context_size - 1
...
class GatingDataset(Dataset):
    def __init__(self, df):
        ...
        for _, row in df.iterrows():
            chunks = prepare_chunks(row["cleaned_text"])
...
with torch.no_grad():
    for batch in tqdm(test_loader, desc="Testing"):
        ...
        lm_labels = input_ids.clone()              # [B, T]

        # === Get gate probabilities ===
        gate_inputs = moe_model.token_embedding_layer(input_ids)  # [B, T, D]
        pooled_inputs = gate_inputs.mean(dim=1)                   # [B, D]
        gate_logits = moe_model.gate(pooled_inputs)               # [B, E]
        ...
        # === Hard gating: use only top-1 expert ===
        selected_expert_idx = torch.argmax(gate_probs, dim=-1)  # [B]
        selected_logits = torch.stack([
            expert_logits[expert_idx, b_idx]
            for b_idx, expert_idx in enumerate(selected_expert_idx)
        ], dim=0)  # [B, T, V]
