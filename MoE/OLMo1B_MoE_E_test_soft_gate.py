...
moe_model.gate.load_state_dict(torch.load("./moe_gate_3epochs_E_epoch3.pt", map_location=device))
moe_model.eval()
...
with torch.no_grad():
    for batch in tqdm(test_loader, desc="Validating"):
        ...
        for i, probs in enumerate(gate_probs_list):
            row_idx = original_indices[i]
            actual_year = test_df.iloc[row_idx]["printedDate"]
...
results_df.to_csv("moe_predictions_E_test.csv", index=False)


#  summary

avg_loss = results_df["loss"].sum() / results_df.shape[0]
avg_perplexity = results_df["perplexity"].sum() / results_df.shape[0]
